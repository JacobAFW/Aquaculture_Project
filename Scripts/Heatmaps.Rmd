...
sample_data(ps3.Microbiome) <- sample_data(ps3.Microbiome) %>%
  ...
  mutate("Sample" = ID) %>% # change rownames to save it back into the original ps object
...
phyloseq_to_deseq2(ps3.Microbiome, ~ par_med) %>% 
    ...
    as.data.frame() %>% 
    rownames_to_column() %>% 
    add_column( ID = c("Salinirepens", "Hyphomonas", "Thalassobaculum", "Salinihabitans",
                       "Marivivens", "Coraliomargarita", "Arcobacter")) %>%  
    column_to_rownames("ID") %>% 
    select(-rowname) %>% 
